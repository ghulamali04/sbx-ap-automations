...
class ReportRequest(BaseModel):
    ...
    def resolved_webhook(self) -> str | None:
        # Task Summary posts a different payload schema (PDF attachment) than the
        # completion-overview flow (chart images), so it needs its own Power
        # Automate flow. Prefer its dedicated URL; fall back to the shared one
        # only for setups that still point both automations at one flow.
        return (
            self.webhook_url
