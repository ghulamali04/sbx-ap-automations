...
async def acquire_token_info() -> dict:
    """Acquire a Graph token and return non-sensitive metadata (for a connectivity test).

    Never returns the token value itself — only proof that it was minted and when
    it expires. Used by the /meeting-notes/graph-test route.
    """
    ...
    return {
        "acquired": True,
        "tenant_id": os.getenv("GRAPH_TENANT_ID") or None,
        "client_id": os.getenv("GRAPH_CLIENT_ID") or None,
        "expires": _dt.datetime.fromtimestamp(expires_on, _dt.timezone.utc).isoformat(),
    }


async def list_user_events(user_id: str, *, top: int = 20) -> list[dict]:
    """List a user's calendar meetings/events, newest first (app-only Calendars.Read).

    A broader 'what meetings does this user have?' view than getAllTranscripts —
    it includes meetings that were never transcribed. A 403 means Calendars.Read is
    not consented on the app registration.
    """
    url = f"{graph_base()}/users/{user_id}/events"
    params = {
        "$select": "subject,start,end,isOnlineMeeting,onlineMeetingProvider,organizer,webLink",
        "$orderby": "start/dateTime desc",
        "$top": top,
    }
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(url, headers=await _auth_headers(), params=params)
    if resp.status_code != 200:
        raise RuntimeError(
            f"events list failed for {user_id} (HTTP {resp.status_code}): {resp.text[:300]}"
        )
    return resp.json().get("value", [])
...
async def list_user_transcripts(
    ...
    *,
    ...
    """All transcripts for meetings organised by this user (app-only getAllTranscripts).

    getAllTranscripts is an OData function: the organiser must be passed as the bound
    parameter meetingOrganizerUserId, with optional startDateTime/endDateTime ISO-8601
    bounds. Requires OnlineMeetingTranscript.Read.All granted with admin consent.
    """
    params = [f"meetingOrganizerUserId='{user_id}'"]
    if start_date_time:
        params.append(f"startDateTime={start_date_time}")
    if end_date_time:
        params.append(f"endDateTime={end_date_time}")
    url = (
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(url, headers=await _auth_headers())
    # getAllTranscripts returns 404 for an organiser who simply has no transcribed
    # meetings — that's an empty result, not a failure.
    if resp.status_code == 404:
        return []
    if resp.status_code != 200:
        raise RuntimeError(
            f"getAllTranscripts failed for {user_id} (HTTP {resp.status_code}): {resp.text[:300]}"
        )
    return resp.json().get("value", [])
