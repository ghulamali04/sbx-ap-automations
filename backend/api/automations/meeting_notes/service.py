...
async def run_note_job(req: NoteJobRequest, job_id: str = "") -> NoteJobResult:
    ...
    if req.transcript_vtt is not None:
        ...
        recording_url = req.recording_url_override or ""
        ...
        try:
            recordings = await graph.list_meeting_recordings(organizer_id, meeting_id)
            if recordings:
                latest = max(recordings, key=lambda r: r.get("createdDateTime") or "")
                recording_url = latest.get("recordingContentUrl") or ""
        except Exception as exc:  # noqa: BLE001 - recording is best-effort, never blocks handoff
            _LOG.warning("Recording fetch failed for %s: %s", meeting_id, exc)
    ...
    try:
        await power_automate.deliver_transcript(
            ...
            # Sent as null for now — no confirmed source/mapping for this field
            # yet. result.business_area (organiser routing) stays populated on
            # the job record for admin visibility.
            business_area=None,
